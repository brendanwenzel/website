import {themes as prismThemes} from 'prism-react-renderer';
import type {Config} from '@docusaurus/types';
import type * as Preset from '@docusaurus/preset-classic';

// This runs in Node.js - Don't use client-side code here (browser APIs, JSX...)

const config: Config = {
  title: 'Brendan Wenzel',
  tagline: 'Director of Social Commerce',
  favicon: 'img/favicon.ico',

  // Set the production url of your site here
  url: 'https://www.brendanwenzel.com',
  // Set the /<baseUrl>/ pathname under which your site is served
  // For GitHub pages deployment, it is often '/<projectName>/'
  baseUrl: '/',

  // GitHub pages deployment config.
  // If you aren't using GitHub pages, you don't need these.
  organizationName: 'brendanwenzel', // Usually your GitHub org/user name.
  projectName: 'website', // Usually your repo name.

  onBrokenLinks: 'throw',
  onBrokenMarkdownLinks: 'warn',

  // Even if you don't use internationalization, you can use this field to set
  // useful metadata like html lang. For example, if your site is Chinese, you
  // may want to replace "en" with "zh-Hans".
  i18n: {
    defaultLocale: 'en',
    locales: ['en'],
  },

  presets: [
    [
      'classic',
      {
        docs: {
          sidebarPath: './sidebars.ts',
          // Please change this to your repo.
          // Remove this to remove the "edit this page" links.
          editUrl:
            'https://github.com/brendanwenzel/website/tree/main/',
        },
        blog: {
          showReadingTime: true,
          feedOptions: {
            type: ['rss', 'atom'],
            xslt: true,
          },
          // Please change this to your repo.
          // Remove this to remove the "edit this page" links.
          editUrl:
            'https://github.com/brendanwenzel/website/tree/main/',
          // Useful options to enforce blogging best practices
          onInlineTags: 'warn',
          onInlineAuthors: 'warn',
          onUntruncatedBlogPosts: 'warn',
        },
        theme: {
          customCss: './src/css/custom.css',
        },
      } satisfies Preset.Options,
    ],
  ],

  themeConfig: {
    // Replace with your project's social card
    image: 'img/brendan-social-card.jpg',
    navbar: {
      title: 'Brendan Wenzel',
      logo: {
        alt: 'Brendan Wenzel Logo',
        src: 'img/profile.jpg',
      },
      items: [
        {
          type: 'docSidebar',
          sidebarId: 'tutorialSidebar',
          position: 'left',
          label: 'Services',
        },
        {to: '/blog', label: 'Blog', position: 'left'},
        {
          href: 'https://www.linkedin.com/in/brendanwenzel/',
          label: 'LinkedIn',
          position: 'right',
        },
      ],
    },
    footer: {
      style: 'dark',
      links: [
        {
          title: 'Services',
          items: [
            {
              label: 'Marketing Consulting',
              to: '/docs/services/marketing-consulting',
            },
            {
              label: 'Media Buying',
              to: '/docs/services/media-buying',
            },
            {
              label: 'Web Development',
              to: '/docs/services/web-development',
            },
          ],
        },
        {
          title: 'Connect',
          items: [
            {
              label: 'LinkedIn',
              href: 'https://www.linkedin.com/in/brendanwenzel/',
            },
            {
              label: 'Email',
              href: 'mailto:contact@brendanwenzel.net',
            },
          ],
        },
        {
          title: 'More',
          items: [
            {
              label: 'Blog',
              to: '/blog',
            },
            {
              label: 'Portfolio',
              to: '/docs/portfolio',
            },
            {
              label: 'Privacy Policy',
              to: '/docs/privacy-policy',
            },
            {
              label: 'Terms of Service',
              to: '/docs/terms-of-service',
            },
          ],
        },
      ],
      copyright: `Copyright ${new Date().getFullYear()} Brendan Wenzel. All rights reserved.`,
    },
    prism: {
      theme: prismThemes.github,
      darkTheme: prismThemes.dracula,
    },
  } satisfies Preset.ThemeConfig,
};

export default config;
